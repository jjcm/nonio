...
const before = await page.evaluate(() => {
  const lists = [...document.querySelectorAll('soci-post-list')]
  lists.forEach((l, i) => { l.__labId = 'pre' + i })
  return lists.map(l => ({ id: l.__labId, tag: l.getAttribute('tag'), rows: l.querySelectorAll('soci-post-li, soci-post-card').length }))
})
...
const trace = await page.evaluate(() => new Promise(res => {
  ...
  const snap = () => {
    ...
    return lists.map(l => ({
      ...
      rows: l.querySelectorAll('soci-post-li, soci-post-card').length,
  ...
  const links = []
  const walk = (root, d) => {
    root.querySelectorAll?.('soci-link').forEach(l => links.push(l))
