...
### iter25: `content-visibility: auto` on below-fold list-view feed cards (2026-08-20 PT) — **REVERT**

Hypothesis: ~12 of the 21 feed cards are appended below the fold (first batch is `ceil(innerHeight/104)` ≈ 9; the rest land via idle callbacks). Skipping layout+style for those off-screen cards (`content-visibility: auto` + `contain-intrinsic-size: 104px`, the lab's per-card height from that same formula) should let the header + first card paint with less layout work, shaving cold FCP (bar 606.4) and possibly cold/warm LCP, at no first-card cost.

Change (1 file, 4 CSS lines, reverted): in `soci-post-list.js` shadow CSS, after the existing `#items::slotted(soci-post-li:first-child)` rule: `:host(:not([view="lanes"])) #items::slotted(soci-post-li:not(:first-child)) { content-visibility: auto; contain-intrinsic-size: 104px; }` — first child explicitly excluded (`:not(:first-child)`), lanes view excluded (it slots `soci-post-card`, not `soci-post-li`). No JS touched: createPosts idle-batching, cache headers, fetch/loading attrs all untouched. `node --check` clean; served :4200 bytes confirmed to carry the rule during the with-change run, and clean after revert; 21-post smoke of `/posts?sort=popular` OK (first 4 = image).

Medians vs iter14 bar (cold 606.4/961.4, warm 217.0/470.1):

| | FCP ms | LCP ms | TBT | score |
|---|---:|---:|---:|---:|
| cold median | 604.4 | 1018.6 | 0 | 0.98 |
| warm median | 217.0 | 489.6 | 0 | 1.00 |

With-change run (3 cold + 3 warm): cold FCP 604.9 / 585.0 / 604.4 (median 604.4, −2.0 = noise); cold LCP 1016.3 / 1057.0 / 1018.6 (median 1018.6, +57.2 — two of three pinned in the ~1016 band, the same pinned-band signature as iter20/iter22 regressions). Warm FCP 217.0 ×3 (exact tie with the bar); warm LCP 489.6 / 450.0 / 530.3 (median 489.6, +19.5 — a three-way spread across the whole 450/490/530 span, median in between; no clean mode win). TBT 0 on every run.

Reading:
1. **Eighth confirmation that below-fold layout skipping does not move a paint whose gate is not below-fold layout.** FCP sits at ~605 in the bar state with the ~119 ms asset-complete + ~500 ms paint gap (iter2 finding) intact; the gap is not spent laying out the 12 idle-appended below-fold cards — they append *after* FCP. Skipping their layout is therefore mechanically inert on cold FCP (−2.0, noise), exactly as predicted by the iter2 paint-gap analysis.
2. **The LCP lane is again insensitive, and the reported LCP node (header `div#placeholder` text, per iter21) never depends on the cards' layout at all.** `content-visibility` on those cards also cannot change the LCP element's paint timing — it only changes layout cost of boxes that don't gate FCP or LCP. +57.2 cold / +19.5 warm medians land inside the documented environmental bands (cold 957–1058, warm 470/530 bimodal; the post-revert confirmation sweep below draws cold LCP 958.2–1055.9 and warm 489.5–530.0 on the *unchanged* code), so even the regression direction is partly environmental — but under the strict faster-on-all-lanes rule the verdict is unambiguous: no lane faster, two LCP median regressions, two tie/noise lanes.
3. **Layout-side levers are now jointly exhausted** with iter17 (first-card box reservation, REVERT) and iter20's confirmation: every CSS layout/sizing intervention on feed cards — reservation, skip, intrinsic-size — is inert because the FCP/LCP gates are the paint schedule itself, not card layout cost. Combined with the closed resource-hint space (iter23/24) and the closed font space (iter22), the only remaining non-forbidden candidate is (b) content-hash filenames — a build-pipeline change, not "small" for the protocol. Recommend closing out at the iter14 state.

Verdict: **REVERT** (strict faster-on-all-lanes rule: cold LCP +57.2, warm LCP +19.5; cold FCP −2.0 noise, warm FCP 217.0 exact tie — no faster lane). Rule removed from `soci-post-list.js`; `git -C soci-frontend diff` clean (submodule at `c43db4f`; served bytes confirmed free of `content-visibility` after revert). Post-revert confirmation sweep reproduces the bar pattern (cold FCP 584.1 / 605.1 / 605.2, median 605.1; cold LCP 1036.5 / 1055.9 / 958.2, median 1036.5; warm FCP 216.9 / 217.0 / 217.0; warm LCP 489.5 / 529.9 / 530.0, median 529.9 — the ~530 warm mode present, reaffirming the 470/530 bimodal is environmental). Keep bar unchanged: cold 606.4/961.4, warm 217.0/470.1 (iter14).

Raw: `speed-lab/metrics/cold-{1..3}.json` + `warm-{1..3}.json` hold the post-revert confirmation sweep; with-change numbers above (per-run raw JSONs superseded by the confirmation sweep, as in iter17/iter20/iter21/iter23/iter24).

### iter26: delay `markdown.wasm` compile/instantiate until after first paint (`requestIdleCallback` gate on `WebAssembly.*`, markdown.js tag left eager) (2026-08-20 PT) — **REVERT**

Hypothesis (the prompt's preferred lever, verbatim): the eager `markdown.js` tag (a 9.7 KB Emscripten loader) synchronously starts the 56 KB `markdown.wasm` `fetch` + `WebAssembly.instantiateStreaming` at script execution time (line 7 of `<head>`, before `soci.js`/`soci-components.js`). Gating `WebAssembly.instantiate`/`instantiateStreaming` on `requestIdleCallback` (timeout 2500, `load`-event fallback) defers the wasm fetch+compile+instantiate off the early main-thread window, with `markdown.js` itself kept eager/blocking (no `defer` — iter3/4 territory). Safe by construction because the only consumer, `soci-markdown-view.js`, awaits `window.markdown.ready` before every `markdown.parse` call; the Emscripten `ready` promise simply resolves later.

Change (1 pug block in `soci-frontend/index.pug`, ≈330 B, reverted): an inline `<script>` immediately before the untouched `script(src="/lib/markdown-wasm/markdown.js")` tag that wraps the two global `WebAssembly` entry points to first `await` a shared `requestIdleCallback` gate. No `defer` added to the markdown.js tag (served bytes confirmed unchanged); no cache-header values touched (iter14 intact); no `markdown.js`/`markdown.wasm` file edits. Live via the per-request `pug.renderFile` server, as in iter22–24. 21-post smoke of `/posts?sort=popular` on :4201 OK (21 posts, first 4 image); served :4200 head confirmed to carry the gate during the with-change run and be free of `requestIdleCallback` after revert.

Medians vs iter14 bar (cold 606.4/961.4, warm 217.0/470.1):

| | FCP ms | LCP ms | TBT | score |
|---|---:|---:|---:|---:|
| cold median | 605.4 | 1057.5 | 0 | 0.98 |
| warm median | 217.0 | 469.7 | 0 | 1.00 |

With-change run (3 cold + 3 warm): cold FCP 605.2 / 585.4 / 605.4 (median 605.4, −1.0 = noise); cold LCP 1058.0 / 1057.5 / 1017.7 (median 1057.5, +96.1 — pinned in the 1018–1058 band). Warm FCP 217.0 / 217.0 / 217.1 (median 217.0, exact tie with the bar); warm LCP 469.5 / 469.7 / 530.3 (median 469.7, −0.4 — two of three in the favorable ~470 mode, one in ~530; a tie). TBT 0 on every run.

Reading:
1. **Mechanistically inert, exactly as the iter21/iter22 findings predict.** The reported LCP node is the header `div#placeholder` text (iter21) and FCP is that same early header paint; neither ever depends on `markdown.wasm`. The wasm fetch+instantiate was already running in parallel behind the HTML→module→feed critical chain and gating nothing, so removing it from the early window can only be a tie or a cost — it cannot be a win. That is precisely the outcome: two exact ties (warm FCP 217.0, warm LCP 469.7) and a cold-LCP regression.
2. **The +96.1 cold-LCP "regression" is environmental, not caused by the gate — but the keep rule is strict and the verdict does not depend on it.** The post-revert confirmation sweep on *unchanged* code reproduces the with-change medians almost exactly (cold FCP 604.9 vs 605.4; cold LCP 1057.5 vs 1057.5; warm FCP 217.0 ×3; warm LCP 470.0 / 529.8 / 530.0). The cold-LCP lane is currently pinning in the 1018–1058 band on any code state (iter25's confirmation sweep drew 958.2–1055.9 on unchanged code; the bar's 961.4 sits at the favorable edge of that same band), so the cold-lane verdict is drawn by which band the sweep lands in, as in iter20/iter22/iter24.
3. **The JS-execution-timing lever family is now closed by measurement, not just by the do-not-retry list.** Script `defer` is dead (iter3/4), `tags.js` preconnect/preload/priority are dead (iter19–23), and now deferring the *intrinsic work* of the one big synchronous JS artifact (wasm instantiate) out of the early window is dead too: post-iter14, no front-end work scheduled in the first ~600 ms gates an FCP/LCP paint. Combined with the closed resource-hint space (iter23/24), font space (iter22), and layout space (iter17/25), this is the ninth consecutive post-iter14 revert. Remaining non-forbidden candidate: (b) content-hash filenames — a build-pipeline change, not "small" for the protocol; otherwise close out at the iter14 state.

Verdict: **REVERT** (strict faster-on-all-lanes rule: no lane faster — cold FCP −1.0 noise, warm FCP 217.0 exact tie, warm LCP −0.4 tie; cold LCP +96.1 pinned-band environmental regression). Gate removed from `index.pug`; `git -C soci-frontend diff` clean (submodule at `c43db4f`; served bytes confirmed free of `requestIdleCallback`, markdown.js tag intact). Post-revert confirmation sweep numbers above. Keep bar unchanged: cold 606.4/961.4, warm 217.0/470.1 (iter14).

Raw: `speed-lab/metrics/cold-{1..3}.json` + `warm-{1..3}.json` hold the post-revert confirmation sweep; with-change numbers above (per-run raw JSONs superseded by the confirmation sweep, as in iter17/iter20/iter21/iter23/iter24/iter26).

### iter27: shrink `#tag-input::placeholder` font (14px→11px) to reduce the LCP node's painted area (2026-08-20 PT) — **REVERT**

Hypothesis (the prompt's lever, verbatim): the reported LCP node is the `#tag-input` placeholder pseudo-element (`div#placeholder`, "Viewing all tags", 238×17 = 4,046 px² at 14px). Reducing its painted area so it is no longer the largest LCP candidate — smaller `::placeholder` font-size, one CSS property, no static input in normal flow (iter18 avoided) — should let the first feed card (96×72 thumbnail = 6,912 px², or the FCP-tick text) become LCP if it paints sooner, or at worst pin the same node at the same time. Monotonicity argument: LCP timestamp is the paint time of the *largest* candidate at finalization; shrinking one candidate's area can only keep its timestamp (tie) or hand the win to a larger-earlier candidate (faster) — it cannot regress. The bet: the FCP-tick contentful element (cold ~605 / warm ~217) is larger than 11px "Viewing all tags" (~238×13 = 3,094 px²) and would inherit LCP.

Change (1 CSS property in `soci-post-list.js` shadow style, ≈16 B, reverted): `#tag-input { &::placeholder { … font-size: 11px; } }`. No cache-header values touched (iter14 intact); no HTML/JS logic change; served :4200 byte confirmed to carry `font-size: 11px` during the with-change run and be free of it after revert. 21-post smoke of `/posts?sort=popular` OK.

Medians vs iter14 bar (cold 606.4/961.4, warm 217.0/470.1):

| | FCP ms | LCP ms | TBT | score |
|---|---:|---:|---:|---:|
| cold median | 604.4 | 1055.2 | 0 | 0.98 |
| warm median | 217.0 | 469.5 | 0 | 1.00 |

With-change run (3 cold + 3 warm): cold FCP 604.2 / 604.4 / 605.6 (median 604.4, −2.0 = noise); cold LCP 1055.2 / 957.9 / 1056.6 (median 1055.2, +93.8 — two of three pinned in the ~1055 band, the same pinned-band signature as iter20/22/26; run 2's 957.9 shows the 958–1058 bimodal cold band is still moving under any code state). Warm FCP 217.0 ×3 (exact tie with the bar); warm LCP 469.2 / 449.5 / 469.5 (median 469.5, −0.6 = tie, all three in the favorable ~450–470 mode). TBT 0 on every run.

Reading:
1. **The bet's premise failed in its simplest form: the FCP-tick contentful element is *smaller* than even the shrunk placeholder.** Post-change traces still report `div#placeholder` (now 238×13 = 3,094 px²) as the LCP node at the *same* paint tick (warm ~469, cold ~958–1056). So no larger-earlier candidate exists to inherit LCP: every contentful element painted before finalization — the FCP-tick text and the placeholder itself — is either the placeholder or smaller than it. Area-reduction of the LCP node is provably inert here, for the opposite reason of iter17/25 (there: the node doesn't gate layout; here: the node is the *only* sufficiently-large candidate before finalization, and demoting it only exposes smaller candidates at the same timestamps).
2. **Why the card never inherits, now measured rather than inferred.** The first feed thumbnail (96×72 = 6,912 px², the true largest candidate on the page) paints only *after* LCP finalization: the feed render is gated on `markdown.ready` (wasm instantiate) + the posts fetch, landing in the same ~958–1056 cold band as the current finalization tick. iter26 already showed deferring the wasm gate cannot move it; iter27 shows that even if you demote everything else out of LCP contention, the card cannot take the seat unless its *paint* moves earlier — which is a feed-pipeline-latency problem, not an LCP-candidate-selection problem. The LCP lane is therefore insensitive to any candidate-area manipulation at this state.
3. **The candidate-area lever family is now closed by measurement** alongside layout (iter17/25), fonts (iter22), resource hints (iter23/24), and JS timing (iter3/4, iter19–26). The strict faster-on-all-lanes rule makes the verdict unambiguous regardless of the environmental cold-LCP band: no lane faster (cold FCP −2.0 noise, warm FCP exact tie, warm LCP −0.6 tie, cold LCP +93.8 pinned-band). Remaining non-forbidden candidate is unchanged: (b) content-hash filenames — a build-pipeline change, not "small" for the protocol; otherwise close out at the iter14 state.

Verdict: **REVERT** (strict faster-on-all-lanes rule: no lane faster — cold FCP −2.0 noise, warm FCP 217.0 exact tie, warm LCP −0.6 tie; cold LCP +93.8 pinned-band environmental regression; LCP node confirmed still `div#placeholder` post-change). `font-size` removed from `#tag-input::placeholder` in `soci-post-list.js`; served :4200 byte confirmed free of `font-size: 11px` on that rule after revert. Post-revert confirmation sweep on unchanged code: cold FCP 604.6 / 605.9 / 604.5 (median 604.5); cold LCP 1017.5 / 999.2 / 1055.8 (median 1017.5); warm FCP 217.0 ×3; warm LCP 449.8 / 469.6 / 530.2 (median 469.6 — the 450/470/530 bimodal warm band intact) — reproducing the bar pattern. Keep bar unchanged: cold 606.4/961.4, warm 217.0/470.1 (iter14).

Raw: `speed-lab/metrics/cold-{1..3}.json` + `warm-{1..3}.json` hold the post-revert confirmation sweep; with-change numbers above (per-run raw JSONs superseded by the confirmation sweep, as in iter17/iter20/iter21/iter23/iter24/iter26).

### iter28: gzip compression on the :4200 static handler for js/css/html/svg (2026-08-20 PT) — **KEEP**

Hypothesis (the prompt's lever, verbatim): the :4200 static server sends every artifact raw — zero `Content-Encoding` produced by either node (`soci-frontend/index.js`) or the backend Go httpd (pre-verified: no gzip/brotli anywhere). The wire per load is ~90.6 KB of text: the 30.3 KB non-cacheable HTML document (refetched on *every* warm pass), the 49 KB `soci.css`, and the 11.3 KB `soci.js`. Gzip via built-in `zlib` should collapse all three (measured after: 8.3 KB / 9.0 KB / 3.6 KB — 3.7×/5.4×/3.1×) at zero client cost (browser-native decompress) and sub-millisecond server cost at these sizes. Every post-iter14 iteration reverted on environmental pinned bands over *unchanged bytes*; changing the bytes is the highest-EV remaining lever class.

Change (`soci-frontend/index.js`, +15/−5, **kept**): `var zlib = require('zlib')`; a `GZIP_EXT` map (`.js`, `.css`, `.html`, `.svg` only — mp4/wasm/webp/binary untouched) and a `maybeGzip(req, res, data)` helper that, when `Accept-Encoding` carries `gzip`, replaces the payload with `zlib.gzipSync(data)` and sets `Content-Encoding: gzip` + `Vary: Accept-Encoding` **before** any writeHead. Applied at the three text send sites: (1) the `handler.file` static path — iter14's `Cache-Control: public, max-age=31536000, immutable` is preserved on the same response; (2) the `handler.folder` index.pug document path (writeHead moved after compression); (3) the SPA fallback `index.pug` document. The mp4 range-request path and the :4201 API are untouched — no binary asset is ever compressed. Pre-measurement smoke: 21 fixture posts; gzip round-trip byte-identical to raw (`curl --compressed` vs plain) for `/` and `/soci.css`; header checks confirm `Content-Encoding: gzip` + `Vary: Accept-Encoding` on js/css with `Accept-Encoding: gzip`, and no `Content-Encoding` without it. Server restarted on :4200 (PID 53575).

Medians vs iter14 bar (cold 606.4/961.4, warm 217.0/470.1):

| | FCP ms | LCP ms | TBT | score |
|---|---:|---:|---:|---:|
| cold median | 427.8 | 802.8 | 0 | 1.00 |
| warm median | 177.0 | 430.1 | 0 | 1.00 |

With-change run (3 cold + prime + 3 warm): cold FCP 428.2 / 427.8 / 427.3 (median 427.8, **−178.6**); cold LCP 824.2 / 802.8 / 797.9 (median 802.8, **−158.6**). Warm prime 427.7/822.6 — cold-shaped, as expected, since the prime refetches the non-cacheable document. Warm FCP 177.0 ×3 (median 177.0, **−40.0**, *zero* spread — the tightest FCP cluster measured in the whole lab); warm LCP 430.1 / 429.9 / 430.2 (median 430.1, **−40.0**). SI cold 435.9/438.6/431.6 (bar 606.4); warm 238.0/242.8/234.1 (bar 284.5); TBT 0 on all seven loads; Lighthouse 0.99/1.00/1.00 cold, 1.00 prime and warms.

Reading:
1. **First structural win since iter14 — the largest single-iteration gain in the lab after iter14 itself.** The pinned-band diagnosis (iter20/22/24/25/26/27) meant no timing lever over unchanged bytes could beat the bar: the only remaining class that *must* beat a byte-identical baseline is shrinking the bytes. ~90.6 KB → ~21.0 KB on the wire moves every lane at once — cold FCP −178.6 / cold LCP −158.6 / warm −40.0/−40.0 — so the strict faster-on-all-lanes rule is met by ~10× the noise margin and the verdict cannot hinge on band lottery.
2. **The pinned-band pathology is gone because it was never a code problem: it was download time.** The 958/1056 bimodal cold-LCP band and the 450/470/530 bimodal warm band are exactly the tail-distribution of ~90 KB crossing a simulated 10 Mbps / ~150 ms RTT uplink. With ~21 KB the cold LCP spread collapses to 797.9–824.2 (26 ms), warm LCP to 429.9–430.2 (0.3 ms), and warm FCP lands on 177.0 three-for-three. Nine consecutive environmental reverts, one byte-level change.
3. **New floors are now visible.** Warm FCP 177.0 is the handoff of the still-non-cacheable ~8.3 KB gzipped document (refetched every warm pass — putting `Cache-Control` on the document is a stale-feed product call, not a small protocol lever); cold is now first-document-fetch + css/js dominated. Next byte levers, if ever pursued: brotli (~5–10% smaller, adds a dependency to built-in zlib) or content-hashed immutable asset names (build-pipeline scale, flagged not "small" since iter25).
4. Tradeoffs accepted: `gzipSync` per request (sub-ms at these sizes); `Vary: Accept-Encoding` only on the four text extensions; iter14 cache semantics untouched (`immutable` rides the same response, no stale-200 interaction at local-dev scale).
