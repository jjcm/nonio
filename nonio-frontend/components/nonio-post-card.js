...
export default class NonioPostCard extends NonioPostLi {
  constructor() {
    super()
  }
  ...
  css() {
    return `
      :host {
        ...
        height: auto;
        transition: opacity 0.25s var(--nonio-ease), transform 0.25s var(--nonio-ease);
      }
      :host([unloaded]) {
        opacity: 0;
  ...
  html() {
    ...
    return `
      <div id="title-row">
        <nonio-link id="internal-link" href="${postPath}">
      ...
      <div id="media">
        <nonio-link id="media-link" href="${postPath}">
          <picture>
            <source class="heic">
            <source class="webp">
      ...
      <div id="body">
        ...
        <div id="details">
          <slot name="user"></slot>
          <nonio-link id="metadata-link" href="${postPath}">
            <span id="votes">${score} vote${score == 1 ? '' : 's'}</span>
            <span id="comments">
              ${NonioIcon?.icon('comments', 16)}
              <span>${comments}</span>
            </span>
            <span id="time">
              ${NonioIcon?.icon('time', 16)}
              <span></span>
            </span>
          </nonio-link>
  ...
  connectedCallback() {
    // Skip nonio-post-li's connectedCallback expand logic, just do the basics
    ...
    const desc = this.querySelector('nonio-markdown-view[slot="description"]')
