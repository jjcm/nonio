...
export default class NonioIcon extends NonioComponent {
  constructor() {
    super()
  }
  ...
  static icon(glyph, size) {
    const { body, viewBox } = NonioIcon._resolve(glyph)
  ...
  attributeChangedCallback(name, oldValue, newValue){
    ...
    if(name === 'glyph') {
      const { body, viewBox } = NonioIcon._resolve(newValue)
...
// - NonioIcon.icon('hash') or NonioIcon('hash')
// - NonioIconClass for direct access to the class if needed
if (typeof window !== 'undefined') {
  window.NonioIconClass = NonioIcon
  window.NonioIcon = (glyph, size) => NonioIcon.icon(glyph, size)
  window.NonioIcon.icon = NonioIcon.icon
}
