...
export default class NonioPostList extends NonioComponent {
  constructor() {
    super()
  ...
  css(){
    return `
      ...
      #tag-input-divider {
        opacity: 0;
        transition: opacity 0.4s var(--nonio-ease);
      }
      ...
      nonio-option[slot="selected"] {
        border-radius: 3px;
        white-space: nowrap;
      }
      nonio-radio-button-group { display: flex; }

      nonio-radio-button-group nonio-radio-button[selected]::after {
        content:'';
        display: block;
        position: absolute;
        top: 32px;
      ...
      :host([loaded]) #items {
        ...
        opacity: 1;
        transition: transform 0.175s cubic-bezier(0.15, 0, 0.2, 1), opacity 0.175s var(--nonio-ease-out);
      }
      #items::slotted(nonio-post-li) {
        margin-top: 8px;
      }
      #items::slotted(nonio-post-li:first-child) {
        margin-top: 0;
      }
      ...
      :host([view="lanes"]) #items[data-grid-lanes-container]::slotted(*) {
        ...
        transform: translateY(6px);
        pointer-events: none;
        transition: opacity 0.125s var(--nonio-ease), transform 0.125s var(--nonio-ease);
      }
  ...
  html(){
    return `
      <scroll-container>
        <content>
          <header>
            <nonio-icon id="menu" glyph="menu" @click=_menuClick></nonio-icon>
            <div id="tag-input-container">
              <input id="tag-input" type="text" placeholder="Viewing all tags" @keydown=_tagKeydown @blur=_tagBlur>
              <nonio-icon id="hash" glyph="hash"></nonio-icon>
            </div>
            <div id="header-spacer"></div>
            <div id="controls">
              <div class="divider" id="tag-input-divider"></div>
              <nonio-select id="sort-select">
                <nonio-option slot="selected" value="popular">Popular</nonio-option>
                <nonio-option value="new">New</nonio-option>
                <nonio-option value="day">Top - Day</nonio-option>
                <nonio-option value="week">Top - Week</nonio-option>
                <nonio-option value="month">Top - Month</nonio-option>
                <nonio-option value="year">Top - Year</nonio-option>
                <nonio-option value="all">Top - All Time</nonio-option>
              </nonio-select>
              <nonio-radio-button-group id="sort-buttons">
                <nonio-radio-button value="popular" selected>popular</nonio-radio-button>
                <nonio-radio-button value="new">new</nonio-radio-button>
                <nonio-radio-button value="week">week</nonio-radio-button>
                <nonio-radio-button value="month">month</nonio-radio-button>
                <nonio-radio-button value="year">year</nonio-radio-button>
                <nonio-radio-button value="all">all</nonio-radio-button>
              </nonio-radio-button-group>
              <div class="divider"></div>
              <nonio-select id="filter-select" dropdown-horizontal-position="right">
                <nonio-option slot="selected" value="all">All</nonio-option>
                <nonio-option value="links">Links</nonio-option>
                <nonio-option value="images">Images</nonio-option>
                <nonio-option value="videos">Videos</nonio-option>
                <nonio-option value="blogs">Blogs</nonio-option>
              </nonio-select>
              <nonio-radio-button-group id="filter-buttons">
                <nonio-radio-button value="all" selected>
                  <nonio-icon glyph="allPosts"></nonio-icon>
                </nonio-radio-button>
                <nonio-radio-button value="images">
                  <nonio-icon glyph="filterImages"></nonio-icon>
                </nonio-radio-button>
                <nonio-radio-button value="videos">
                  <nonio-icon glyph="filterVideos"></nonio-icon>
                </nonio-radio-button>
                <nonio-radio-button value="blogs">
                  <nonio-icon glyph="filterBlogs"></nonio-icon>
                </nonio-radio-button>
                <nonio-radio-button value="links">
                  <nonio-icon glyph="filterLinks"></nonio-icon>
                </nonio-radio-button>
              </nonio-radio-button-group>
              <div class="divider"></div>
              <nonio-radio-button-group id="view-buttons">
                <nonio-radio-button value="list" title="List view" selected>
                  <nonio-icon glyph="viewList"></nonio-icon>
                </nonio-radio-button>
                <nonio-radio-button value="lanes" title="Grid lanes view">
                  <nonio-icon glyph="viewLanes"></nonio-icon>
                </nonio-radio-button>
              </nonio-radio-button-group>
            </div>
          </header>
  ...
  _menuClick(){
    document.querySelector('nonio-sidebar')?.toggleAttribute('overlay', true)
  }

  _initializeControls(){
    this._initializing = true
    const savedSort = localStorage.getItem('nonio-column-sort')
    const savedFilter = localStorage.getItem('nonio-column-filter')
    const savedView = localStorage.getItem('nonio-column-view')
  ...
  _syncSelectValue(select, value){
    if(!select) return
    const options = Array.from(select.querySelectorAll('nonio-option'))
  ...
  _sortChanged(){
    ...
    localStorage.setItem('nonio-column-sort', this.getAttribute('sort') || 'popular')
  }

  _filterChanged(){
    this.setAttribute('filter', this.select('#filter-select')?.value)
    localStorage.setItem('nonio-column-filter', this.getAttribute('filter') || 'all')
  }
  ...
  _sortGroupChanged(e){
    ...
    if(sort) {
      this.setAttribute('sort', sort)
      localStorage.setItem('nonio-column-sort', sort)
    }
  }
  ...
  _filterGroupChanged(e){
    ...
    if(filter) {
      this.setAttribute('filter', filter)
      localStorage.setItem('nonio-column-filter', filter)
    }
  }
  ...
  _viewGroupChanged(e){
    ...
    if(!view) return
    this.setAttribute('view', view)
    localStorage.setItem('nonio-column-view', view)
  }
  ...
  _navigateToTag(){
    ...
    const community = this.getAttribute('community') || window.nonio?.routeContext?.community
  ...
  _syncFromLocation(){
    ...
    this._syncTagInput()
    if(!/^\/user\//.test(window.location.pathname || '')) document.querySelector('nonio-sidebar')?.activateTag(tag)
  }
  ...
  // Upvote chrome is stamped at render time from nonio.votes; when /votes
  ...
  _applyVotes(){
    const votes = window.nonio?.votes || {}
    this.querySelectorAll('nonio-post-li, nonio-post-card').forEach(post => {
      const upvoted = votes[post.getAttribute('post-id')]
      if(!upvoted?.length) return
      post.querySelectorAll('nonio-tag[tag-id]').forEach(tag => {
  ...
  async fetchAndMerge(url){
    ...
    try {
      ...
      const domIds = new Set(
        Array.from(this.querySelectorAll('nonio-post-li, nonio-post-card'))
      ...
      newPosts.forEach((post, i) => {
        ...
        setTimeout(() => {
          postEl.style.transition = 'opacity 0.3s var(--nonio-ease), transform 0.3s var(--nonio-ease)'
  ...
  _masonryFlashDebug(label){
    if(localStorage.getItem('nonio-debug-masonry-flash') !== '1') return
    if(!this._items) return

    const children = Array.from(this.querySelectorAll('nonio-post-li, nonio-post-card'))
    ...
    console.log('[nonio-post-list][masonry-flash]', {
  ...
  renderPostLi(post, i){
    ...
    return`
      <nonio-post-li ${i !== undefined && i < 12 ? 'eager ' : ''}post-title="${post.title.replaceAll('"', '&quot;')}" url="${post.url}" post-id="${post.ID}" score=${post.score || 0} comments=${post.commentCount || 0} type=${post.type || 'image'} time=${post.time} ${post.link ? `link=${post.link}` : ''} ${post.community ? `community="${post.community}"` : ''}>
        <nonio-user name="${post.user}" slot="user"></nonio-user>
        <nonio-tag-group slot="tags">
          ${this.sortTags(post.tags).map(tag => `<nonio-tag tag="${tag.tag}" score="${tag.score}" tag-id="${tag.tagID}" ${nonio.votes[post.ID]?.includes(tag.tagID) ? 'upvoted':''}></nonio-tag>`).join('')}
        </nonio-tag-group>
      </nonio-post-li>
    `
  }
  ...
  renderPostCard(post){
    const desc = post.content ? `<nonio-markdown-view slot="description" markdown="${this._escapeAttr(post.content)}"></nonio-markdown-view>` : ''
    return`
      <nonio-post-card post-title="${post.title.replaceAll('"', '&quot;')}" url="${post.url}" post-id="${post.ID}" score=${post.score || 0} comments=${post.commentCount || 0} type=${post.type || 'image'} time=${post.time} ${post.link ? `link="${post.link}"` : ''} ${post.community ? `community="${post.community}"` : ''}>
        ${desc}
        <nonio-user name="${post.user}" slot="user"></nonio-user>
        <nonio-tag-group slot="tags">
          ${this.sortTags(post.tags).map(tag => `<nonio-tag tag="${tag.tag}" score="${tag.score}" tag-id="${tag.tagID}" ${nonio.votes[post.ID]?.includes(tag.tagID) ? 'upvoted':''}></nonio-tag>`).join('')}
