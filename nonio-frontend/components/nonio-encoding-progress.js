...
export default class NonioEncodingProgress extends NonioComponent {
  constructor() {
    super()
  }
      ...
      <columns>
        <column>
          <div class="fidelity" resolution="4320p" disabled>
            <nonio-radial-progress percent="0"></nonio-radial-progress>
        ...
        <column>
          <div class="fidelity" resolution="1080p" disabled>
            <nonio-radial-progress percent="0" waiting></nonio-radial-progress>
  ...
  setResolution(width, height) {
    console.log('[NonioEncodingProgress] setResolution called:', width, height)
    ...
    console.log('[NonioEncodingProgress] Equivalent resolution:', this._equivalentResolution)
    ...
    if(!this._firstResolutionReceived) {
      console.log('[NonioEncodingProgress] First resolution received, checking for lower resolutions to mark complete')
  ...
  _markLowerResolutionsComplete() {
    console.log('[NonioEncodingProgress] _markLowerResolutionsComplete called')
    ...
    for(let i = 0; i < resolutionOrder.length; i++) {
      ...
      const progress = this.select(`[resolution="${res}"] nonio-radial-progress`)
      if(progress) {
        console.log(`[NonioEncodingProgress] Checking ${res}: percent = ${progress.percent}`)
        if(progress.percent > 0) {
          firstActiveIndex = i
          console.log(`[NonioEncodingProgress] Found first active resolution: ${res} at index ${i}`)
          break
        }
      }
    }
    ...
    if(firstActiveIndex > 0) {
      console.log(`[NonioEncodingProgress] Marking resolutions 0-${firstActiveIndex-1} as complete`)
      for(let i = 0; i < firstActiveIndex; i++) {
        const res = resolutionOrder[i]
        const progress = this.select(`[resolution="${res}"] nonio-radial-progress`)
        if(progress && progress.percent === 0) {
          console.log(`[NonioEncodingProgress] Marking ${res} as 100% complete`)
          progress.toggleAttribute('waiting', false)
          progress.percent = 100
        }
      }
    } else {
      console.log('[NonioEncodingProgress] No active resolution found, nothing to mark as complete')
    }
  }

  // Method to update progress for a specific resolution
  updateProgress(resolution, percent) {
    console.log(`[NonioEncodingProgress] updateProgress called: resolution=${resolution}, percent=${percent}, _firstProgressReceived=${this._firstProgressReceived}`)
    const res = resolution === 'source' ? this._equivalentResolution : resolution
    console.log(`[NonioEncodingProgress] Resolved resolution: ${res}`)
    const progress = this.select(`[resolution="${res}"] nonio-radial-progress`)
    if(progress) {
      progress.toggleAttribute('waiting', false)
      progress.percent = percent
      console.log(`[NonioEncodingProgress] Updated ${res} progress to ${percent}%`)
      ...
      if(!this._firstProgressReceived) {
        console.log(`[NonioEncodingProgress] First progress update received for ${res}, checking if lower resolutions should be marked complete`)
      ...
      console.warn(`[NonioEncodingProgress] Could not find progress element for resolution: ${res}`)
    }
  }
  ...
  _markLowerResolutionsCompleteFromCurrent(currentRes) {
    console.log(`[NonioEncodingProgress] _markLowerResolutionsCompleteFromCurrent called with: ${currentRes}`)
    const resolutionOrder = ['480p', '720p', '1080p', '1440p', '2160p', '4320p']
    const currentIndex = resolutionOrder.indexOf(currentRes)
    console.log(`[NonioEncodingProgress] Current resolution index: ${currentIndex}`)
    
    // If we're encoding a resolution that's not the first one (480p),
    // all lower resolutions must be complete
    if(currentIndex > 0) {
      console.log(`[NonioEncodingProgress] Marking resolutions 0-${currentIndex-1} as complete`)
      for(let i = 0; i < currentIndex; i++) {
        const res = resolutionOrder[i]
        const progress = this.select(`[resolution="${res}"] nonio-radial-progress`)
        if(progress) {
          console.log(`[NonioEncodingProgress] Checking ${res}: current percent = ${progress.percent}`)
          ...
          console.warn(`[NonioEncodingProgress] Could not find progress element for ${res}`)
        }
      }
    } else {
      console.log(`[NonioEncodingProgress] Current resolution is 480p (index 0), nothing to mark as complete`)
    }
  }
}
