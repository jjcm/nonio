...
export default class NonioUserComment extends NonioComponent {
  constructor() {
    super()
  }
  ...
  css(){
    return `
      ...
      ::slotted(nonio-comment) {
        padding-left: 4px;
        border-radius: 4px;
        transition: padding 0.1s var(--nonio-ease);
      }

      :host([unread]) ::slotted(nonio-comment) {
        padding: 8px 12px;
  ...
  attributeChangedCallback(name, oldValue, newValue){
    switch(name) {
      ...
      case 'post-title':
        ...
        break
      default:
        this.querySelector('nonio-comment').setAttribute(name, newValue)
        break
    }
  }
  ...
  factory(user, score, lineageScore, date, id, content, edited, postUrl, postTitle, unread){
    let userComment = document.createElement('nonio-user-comment')
