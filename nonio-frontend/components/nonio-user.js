...
export default class NonioUser extends NonioComponent {
  ...
  html(){ 
    let name = this.hasAttribute('self') ? nonio.username : this.getAttribute('name')
    return `
    <nonio-link ${name ? `href="/user/${name}"` : ''}>
      <div>
  ...
  attributeChangedCallback(name, oldValue, newValue){
    switch(name) {
      case 'name':
        ...
        this.select('nonio-link').setAttribute('href', `/user/${newValue}`)
  ...
  _updateUser(){
    if(this.hasAttribute('self') && nonio.username){
      this.setAttribute('name', nonio.username)
    }
  }

  _updateAvatar(){
    if(this.hasAttribute('self')){
      this.select('picture').innerHTML = this._setImages(nonio.username, true)
    }
  }
