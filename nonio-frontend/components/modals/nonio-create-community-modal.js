class NonioCreateCommunityModal extends HTMLElement {
  connectedCallback(){
    ...
    this.innerHTML = `
      <form id="create-community-form" class="modal-form">
        ...
        <nonio-button async id="submit-create-community">Create</nonio-button>
      </form>
    `
  ...
  async _submit(){
    ...
    if(!sidebar) return
    if(!sidebar.authToken) return window.nonio?.requireLogin?.('create a community')
    ...
    try {
      ...
      window.nonioModals?.close('createCommunity')
      if(result?.url) {
...
export default NonioCreateCommunityModal
