...
class NonioImageViewerModal extends HTMLElement {
  constructor() {
    super()
    this._imageUrls = []
    this._index = 0
  ...
  _onKeydown(e) {
    const modal = this.closest('nonio-modal')
    ...
    if (e.key === 'Escape') {
      e.preventDefault()
      window.nonioModals?.close('imageViewer')
    }
  }
...
export default NonioImageViewerModal
