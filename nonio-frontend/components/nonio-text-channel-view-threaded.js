...
export default class NonioTextChannelViewThreaded extends NonioComponent {
  constructor() {
    super()
      ...
      <div id="layout">
        <div id="main">
          ...
          <div id="main-compose" class="compose">
            <div class="compose-row">
              <nonio-input id="message-input" placeholder="Message..."></nonio-input>
              ...
              <button id="attach-btn" class="icon-btn" type="button" title="Attach image">
                <nonio-icon glyph="filterImages" size="16"></nonio-icon>
              </button>
        ...
        <div id="thread">
          ...
          <div class="compose" style="left:10px;right:10px;">
            <div class="compose-row">
              <nonio-input id="thread-input" placeholder="Reply in thread..."></nonio-input>
  ...
  _renderMessage(msg, compact = false) {
    ...
    const row = document.createElement('nonio-message-row')
    ...
    const md = document.createElement('nonio-markdown-view')
  ...
  _emojiNode(emoji) {
    ...
    if (m) {
      const name = m[1]
      const customEmoji = document.createElement('nonio-emoji')
  ...
  async _sendMessage() {
    ...
    const sent = this._normalizeMessage({
      id: res.id,
      user: this._resolveMessageUser(res, window.nonio?.username || ''),
  ...
  async _sendThreadReply() {
    ...
    input.value = ''
    this._clearPendingAttachments('thread')
    const replyUser = this._resolveMessageUser(res, window.nonio?.username || '')
        ...
        if (isCustom) {
          return `
            <button class="emoji-btn" data-emoji=":${item.name}:" title=":${item.name}:">
              <nonio-emoji class="chip-emoji" name="${item.name}"></nonio-emoji>
            </button>
          `
        } else {
          return `
  ...
  _onEmojiContext(e) {
    const path = e.composedPath?.() || []
    const emojiNode = path.find((node) => node?.tagName === 'NONIO-EMOJI')
      || e.target?.closest?.('nonio-emoji')
  ...
  _incrementMainReplyCount(messageID) {
    ...
    const row = this.querySelector(`nonio-message-row[data-message-id="${messageID}"]`)
  ...
  _replaceRenderedReactions(messageID) {
    ...
    if (!msg) return
    this.querySelectorAll(`nonio-message-row[data-message-id="${messageID}"]`).forEach((row) => {
  ...
  async _openImageViewer(imageUrls, index = 0) {
    const modal = await window.nonioModals?.open?.('imageViewer')
    if (!modal) return
    const viewer = modal.querySelector('nonio-image-viewer-modal')
    if (!viewer) return
  ...
  _syncRenderedMessageRow(message) {
    ...
    const rows = Array.from(this.querySelectorAll(`nonio-message-row[data-message-id="${message.id}"]`))
    ...
    const md = row.querySelector('nonio-markdown-view')
    if (md) {
      if (message.content) {
        md.style.display = ''
