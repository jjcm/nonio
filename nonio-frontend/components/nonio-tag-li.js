...
export default class NonioTagLi extends NonioComponent {
  constructor() {
    super()
  }
  ...
  css(){
    return `
      ...
      :host([load-in]){
        animation: load-in 0.2s var(--nonio-ease) forwards;
      }

      :host([load-in][subscribed]) a {
        animation: load-in-up 0.2s var(--nonio-ease) forwards;
      }
      :host([load-in]:not([subscribed])) a {
        animation: load-in-down 0.2s var(--nonio-ease) forwards;
      }

      :host([load-out]){
        animation: load-out 0.2s var(--nonio-ease) forwards;
      }

      :host([load-out][subscribed]) a {
        animation: load-out-down 0.2s var(--nonio-ease) forwards;
      }
      :host([load-out]:not([subscribed])) a {
        animation: load-out-up 0.2s var(--nonio-ease) forwards;
      }
  ...
  _toggleSubscribe(e){
    ...
    this.postData(`/subscription/${subscribing ? 'create' : 'delete'}`, {
      tag: this.innerHTML,
      community: window.nonio.routeContext.community
    })
  }
