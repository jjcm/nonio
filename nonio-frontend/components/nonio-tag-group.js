...
export default class NonioTagGroup extends NonioComponent {
  constructor() {
    super()
  }
  ...
  css(){ return `
    ...
    #add-tag[active] {
      transition: all 0.1s var(--nonio-ease-out);
      width: 150px;
  ...
  connectedCallback(){
    ...
    this.toggleAttribute('upvoted', this.querySelectorAll('nonio-tag[upvoted]').length)
  ...
  addTag(){
    if(!this.authToken) return window.nonio?.requireLogin?.('add tags to posts')
    let input = this.select('#add-tag input')
    let tagName = input.value
    console.log(tagName)
    // Check if this tag already exists on the post
    let existingTag = Array.from(this.querySelectorAll('nonio-tag')).find(tag=>tag.getAttribute('tag')==tagName)
    if(existingTag){
      existingTag.vote()
    ...
    const community = this.closest('[community]')?.getAttribute('community') || window.nonio.routeContext.community || ''
    ...
    this.postData('/posttag/create', {
      ...
      if(res.postID && res.tagID)
        nonio.votes[res.postID]?.push(res.tagID)
    })

    if(this.childNodes[0]?.nodeType == 3) this.childNodes[0].remove()
    this.innerHTML = `<nonio-tag tag="${tagName}" score="1" upvoted></nonio-tag>` + this.innerHTML
  ...
  _tagVoted(e){
    let numberOfUpvotes = this.querySelectorAll('nonio-tag[upvoted]').length
