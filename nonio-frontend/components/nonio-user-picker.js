...
export default class NonioUserPicker extends NonioComponent {
  constructor() {
    super()
    ...
    return `
      <div class="wrapper">
        <div class="field">
          ...
          <nonio-button type="button" @click=_submit>${label}</nonio-button>
        </div>
        <ul id="results"></ul>
      </div>
    `
  ...
  _renderResults(query) {
    ...
    suggestions.forEach((user, idx) => {
      const item = document.createElement('nonio-user')
  ...
  _setSelection(index) {
    const items = Array.from(this._list.querySelectorAll('nonio-user'))
