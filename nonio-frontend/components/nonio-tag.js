...
export default class NonioTag extends NonioComponent {
  ...
  css(){
    return `
      :host {
        ...
        font-weight: 600;
        user-select: none;
        //transition: background 0.1s var(--nonio-ease-out);
        margin-right: 6px;
        overflow: hidden;
        white-space: nowrap;
        --fill-color: none;
  ...
  attributeChangedCallback(name, oldValue, newValue){
    ...
    switch(name){
      ...
      case 'tag':
        ...
        this.select('nonio-link').innerHTML = newValue
    }
  }
  ...
  html(){ 
    ...
    <nonio-link href="${this._tagHref()}">${this.getAttribute('tag')}</nonio-link>
  `}
  ...
  set tag(val){
    ...
    this.select('nonio-link').href = this._tagHref(val)
  }
  
  get community(){
    return this.closest('[community]')?.getAttribute('community') 
      || this.closest('nonio-post')?.getAttribute('community') 
      || window.nonio.routeContext.community 
      || ''
  }
  ...
  vote(e){
    e?.preventDefault()
    if(!this.authToken) return window.nonio?.requireLogin?.('upvote tags')
    ...
    if(post) {
      const community = post.getAttribute('community') || window.nonio.routeContext.community || ''
      this.postData(`/posttag/${upvoted ? 'add' : 'remove'}-vote`, {
        post: post.getAttribute('url'),
        tag: this.tag,
        community
      }).then(()=>{
        nonio.votes[post.getAttribute('post-id')].push(parseInt(this.getAttribute('tag-id')))
      })
    }
    else {
