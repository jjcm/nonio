...
export default class NonioPostLi extends NonioComponent {
  ...
  css(){
    return `
      ...
      :host([expanded]) #title {
        ...
        line-height: 28px;
        white-space: normal;
        animation: load-in 0.2s var(--nonio-ease) 0.14s forwards;
        opacity: 0;
        margin-top: 4px;
        margin-bottom: 12px;
      }
      ...
      :host([expanded]) ::slotted(nonio-markdown-view) {
        display: block;
        opacity: 0;
        animation: load-in 0.25s var(--nonio-ease) 0.14s forwards;
        font-size: 14px;
      }
  ...
  html(){ 
    ...
    <content>
      <div id="top">
        <div id="details">
          <slot name="user"></slot>
            <nonio-link id="metadata-link" ${url ? `href="${postPath}"` : ''}>
            <div id="votes">${score} vote${score == 1 ? '' : 's'}</div>
            <div id="comments">${window.NonioIcon?.('comments', 16) || ''}<span>${comments} comment${comments == 1 ? '' : 's'}</span></div>
            <div id="time">${window.NonioIcon?.icon?.('time', 16) || ''}<span></span><suffix> ago</suffix></div>
            <div id="domain">${link?.replace(/^(?:https?:\/\/)?(?:www\.)?([^\/]+).*$/, '$1')}</div>
          </nonio-link>
          <div id="delete" style="display: none;" @click=deletePost><span>delete</span></div>
        </div>
          <nonio-link id="internal-link" ${url ? `href="${postPath}"` : ''}>
  ...
  _checkDeletePermission(){
    const author = this.querySelector('nonio-user')?.getAttribute('name')
  ...
  expand(){
    ...
    if(this.hasAttribute('expanded')){
      ...
      let description = document.createElement('nonio-markdown-view')
      ...
      this.querySelector('nonio-markdown-view')?.remove()
