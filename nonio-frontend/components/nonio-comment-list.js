...
export default class NonioCommentList extends NonioComponent {
  ...
  css(){
    return `
      ...
      comment-input {
        ...
        margin: 0 auto;
        &[active] {
          ::slotted(nonio-input) { --min-height: 200px !important; margin-bottom: 8px; }
      ...
      button-container {
        ...
        position: relative;
        padding-left: 12px;
        transition: all 0.1s var(--nonio-ease);
        max-width: 840px;
        margin: 0 auto;
      }
      ::slotted(nonio-comment) { padding-left: 0; }
    `
  }
  ...
  html(){ return `
      ...
      <comment-input>
        ...
        <button-container>
          <nonio-button @click=cancelComment subtle>cancel</nonio-button>
          <nonio-button id="submit" @click=addComment async>submit</nonio-button>
        </button-container>
      </comment-input>
      <content>
        <slot></slot>
  ...
  connectedCallback(){
    let commentInput = document.createElement('nonio-input')
  ...
  _onFocus(){
    ...
    this.querySelector('nonio-input').toggleAttribute('active', true)
  }

  cancelComment(){
    this.querySelector('nonio-input').clear()
    this.select('comment-input').toggleAttribute('active', false)
    this.querySelector('nonio-input').toggleAttribute('active', false)
  }
  ...
  async renderComments(url){
    ...
    comments.forEach(comment => {
      let newComment = document.createElement('nonio-comment')
      ...
      let parent = comment.parent > 0 ? this.querySelector(`nonio-comment[comment-id="${comment.parent}"]`) : this
  ...
  addComment(){
    let value = this.querySelector('nonio-input').value
