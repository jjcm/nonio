...
export default class NonioAvatarUploader extends NonioComponent {
  ...
  get _cropConfig() {
    return NonioAvatarUploader.CROP_TYPES[this.getAttribute('type')] || NonioAvatarUploader.CROP_TYPES.avatar
  }
  ...
  css(){
    return `
      :host {
        ...
        cursor: pointer;
        border-radius: 8px;
        transition: margin-bottom 0.2s var(--nonio-ease);
      }
  ...
  html(){ return `
    ...
    <div id="actions">
      <nonio-button async @click=_upload>submit</nonio-button>
      <nonio-button subtle @click=_cancelEditing>cancel</nonio-button>
    </div>
  `}
  ...
  async _upload(){
    ...
    const btn = this.select('nonio-button[async]')
