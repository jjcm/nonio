...
export default class NonioVideoPlayer extends NonioComponent {
  constructor() {
    super()
  }
  ...
  css(){ return `
    ...
    controls {
      ...
      bottom: 0;
      opacity: 0;
      transition: opacity 0.2s var(--nonio-ease);
    ...
    nonio-icon[glyph="exitfullscreen"],
    :host(:fullscreen) nonio-icon[glyph="fullscreen"] {
      display: none;
    }
    :host(:fullscreen) nonio-icon[glyph="exitfullscreen"] {
      display: block;
    }
    ...
    #volume-container {
      ...
      align-items: center;
      width: 36px;
      transition: all 0.2s var(--nonio-ease);
      padding-right: 0;
      position: relative;
      overflow: hidden;
    }
    ...
    nonio-select:hover nonio-option[slot="selected"] {
      opacity: 1;
    }
    nonio-select[disabled] {
      pointer-events: none;
    }
  `}
  ...
  html(){ return `
    ...
    <controls>
      <nonio-icon id="play" glyph="play" @click=_togglePlay></nonio-icon>
      <div id="volume-container">
        <nonio-icon glyph="volume" @click=_toggleMute></nonio-icon>
      ...
      <nonio-select id="resolution" dropdown-vertical-position="top" dropdown-horizontal-position="right">
      </nonio-select>
      <nonio-icon glyph="fullscreen" @click=_fullscreen></nonio-icon>
      <nonio-icon glyph="exitfullscreen" @click=_exitFullscreen></nonio-icon>
    </controls>
  ...
  _volumeDown(e){
    ...
    if(this.hasAttribute('muted')){
      this.select('#volume-container nonio-icon').setAttribute('glyph', 'volume')
  ...
  _toggleMute(e){
    ...
    if(this.hasAttribute('muted')){
      this.select('#volume-container nonio-icon').setAttribute('glyph', 'muted')
  ...
  set resolution(res){
    ...
    this.select('nonio-option[slot="selected"]')?.removeAttribute('slot')
    ...
    this.select(`nonio-option[value="${res}"]`).setAttribute('slot', 'selected')
  }
      ...
      if(width && height){
        ...
        for(let res in resolutionBreakpoints) {
          if(resolution > resolutionBreakpoints[res] || res == "480p") {
            ...
            resOption = document.createElement('nonio-option')
