...
export default class NonioComment extends NonioComponent {
  constructor() {
    super()
  }
  ...
  css(){
    return `
      :host {
        ...
        margin: 12px 0 8px;
        min-height: 0px;
        transition: height 0.1s var(--nonio-ease);
      ...
      nonio-user { color: var(--text-brand); }
      .edited, time {
      ...
      #upvote {
        ...
        margin-right: 2px;
        nonio-icon { color: var(--text-secondary); height: 20px; width: 20px; }
        ...
        &[upvoted] {
          ...
          nonio-icon { color: transparent; }
        }
      }
      nonio-icon[glyph=downvote] {
        cursor: pointer;
        color: var(--text-secondary);
        --fill-color: transparent;
        height: 20px;
      ...
      actions nonio-button:last-child { margin-right: 0; }
      ::slotted(nonio-input:not([readonly])) {
  ...
  html(){ return `
    <comment>
      <top>
        <nonio-user size="small"></nonio-user>
        <div id="vote-container">
          <div id="upvote" @click=_upvote>
            <nonio-icon glyph="upvote"></nonio-icon>
            <div id="score"></div>
          </div>
          <nonio-icon glyph="downvote" @click=_downvote></nonio-icon>
        </div>
        <time>0s ago</time>
  ...
  attributeChangedCallback(name, oldValue, newValue){
    switch(name) {
      case 'score':
        ...
        break
      case 'user':
        this.select('nonio-user').setAttribute('name', newValue)
        this.toggleAttribute('self', newValue == nonio.username) 
        break
      case 'date':
  ...
  connectedCallback(){
    let numberOfReplies = this.querySelectorAll('nonio-comment').length
    if(numberOfReplies)
      this.select('#view-replies').innerHTML = `view ${this.querySelectorAll('nonio-comment').length} replies`
    ...
    if(!this.quillView){
      this.quillView = document.createElement('nonio-markdown-view')
      this.quillView.setAttribute('slot', 'content')
      this.appendChild(this.quillView)
    }
    this._renderContent()

    let user = this.select('nonio-user')
    user.toggleAttribute('op', user.getAttribute('name') == this.closest('nonio-post')?.getAttribute('user')) 
  }
  ...
  get community(){
    return this.closest('nonio-comment-list')?.getAttribute('community') || window.nonio.routeContext.community || ''
  }

  factory(user, score, lineageScore, date, id, content, edited){
    let comment = document.createElement('nonio-comment')
  ...
  _renderContent(){
    let contentContainer = this.querySelector('nonio-markdown-view[slot="content"]')
    contentContainer?.render(this._content)
  }

  _reply(){
    let replyInput = document.createElement('nonio-input')
    ...
    replyContainer.innerHTML = `
      ...
      <actions>
        <nonio-button>submit</nonio-button>
        <nonio-button subtle>cancel</nonio-button>
      </actions>`
    replyContainer.querySelector('nonio-button').addEventListener('click', this._submitReply.bind(this))
    replyContainer.querySelector('nonio-button[subtle]').addEventListener('click', this._cancelReply.bind(this))
    ...
    setTimeout(()=>{
      ...
      this.querySelector('nonio-input')?.focus()
    }, 100)
  ...
  _cancelReply(){
    ...
    this.querySelector('nonio-input').remove()
  }
  ...
  _toggleReplies(e){
    ...
    if(this.hasAttribute('expanded')){
      ...
      button.innerHTML = `view ${this.querySelectorAll('nonio-comment').length} replies`
    }
  }
  ...
  showVote(upvote){
    if(upvote){
      ...
      let downvote = this.select('nonio-icon[glyph="downvote"]')
  ...
  _submitReply(){
    this.postData('/comment/create', {
      post: this.url,
      content: this.querySelector('nonio-input').value,
  ...
  _edit(){
    let editInput = document.createElement('nonio-input')
    editInput.setAttribute('placeholder', 'Enter comment')
    editInput.setAttribute('slot', 'edit')
    this.appendChild(editInput)
    editInput.value = this.querySelector('nonio-markdown-view').value
    ...
    editContainer.innerHTML = `
      <slot name="edit"></slot>
      <actions>
        <nonio-button async>save</nonio-button>
        <nonio-button subtle>cancel</nonio-button>
      </actions>`
    editContainer.querySelector('nonio-button').addEventListener('click', this._submitEdit.bind(this))
    editContainer.querySelector('nonio-button[subtle]').addEventListener('click', this._cancelEdit.bind(this))
    editContainer.classList.add('active')
    let input = this.querySelector('nonio-input')
    setTimeout(()=>{
  ...
  _submitEdit(){
    let content = this.querySelector('nonio-input').value
    this.postData('/comment/edit', {
      id: parseInt(this.getAttribute('comment-id')),
      content: content
    }).then(()=>{
      this.select('#comment-edit nonio-button').success()
      setTimeout(()=>{
        this.querySelector('nonio-markdown-view').value = content
        this._cancelEdit()
      }, 1000)
    })
  }
