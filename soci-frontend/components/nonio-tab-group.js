...
export default class NonioTabGroup extends NonioComponent {
  constructor() {
    super()
  }
  ...
  _createTabs(){
    let tabs = Array.from(this.querySelectorAll('nonio-tab'))
    return `
      ${tabs.map((tab, i) => {
  ...
  async activateTab(tabName){
    const tabs = Array.from(this.querySelectorAll('nonio-tab'))
