import { getActiveTokenQuery, scanRichTextTokens } from '../lib/nonio-rich-text.js'

export default class NonioInput extends HTMLElement {
    ...
    this.innerHTML = `
      <style>
        ...
        .md-input {
          ...
          width: 100%;
          min-height: var(--min-height);
          transition: min-height 0.1s ease-out, padding 0.1s var(--nonio-ease);
          padding: var(--padding);
          font: inherit;
          line-height: 1.42;
          color: var(--text);
      ...
      if (item.type === 'emoji') {
        return `
          <li data-index="${idx}" ${idx === 0 ? 'selected' : ''}>
            <nonio-emoji name="${item.value}"></nonio-emoji>
