...
export default class NonioLedgerMonth extends NonioComponent {
  constructor() {
    super()
  }
  ...
  css(){
    return `
      ...
      #number, #date, nonio-icon { color: var(--text-secondary); }
      ...
      ::slotted(nonio-ledger-li) { border: 0; animation: load-down 0.15s var(--nonio-ease); }
    `
  }

  html(){ return `
    <div id="header" @click=_toggleContributions>
      <nonio-icon glyph="create"></nonio-icon>
  ...
  _toggleContributions(){
    if(this.classList.toggle('open')) {
      this.select('#header nonio-icon').setAttribute('glyph', 'remove')
    }
    else {
      this.select('#header nonio-icon').setAttribute('glyph', 'create')
    }
  }
}