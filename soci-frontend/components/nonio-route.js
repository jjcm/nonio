...
export default class NonioRoute extends NonioComponent {
  constructor() {
    super()
  }
  ...
  connectedCallback(){
    ...
    */
    ...
    let parentRoute = this.parentElement.closest('nonio-route')
    if(parentRoute) {
  ...
  activate(fresh){
    ...
    /*
    this.innerHTML.match(/<nonio-(\w+)[^>]*>/g).forEach(tag => {
      let name = tag.match(/<nonio-(\w+)[^>]*>/)[1]
      console.log(name)
      import(`./nonio-${name}.js`).then(module => {
        window.customElements.define(`nonio-${name}`, module.default)
      })
