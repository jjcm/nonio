export default class NonioRouter {
  ...
  route(path, fresh){
    document.querySelectorAll('nonio-route').forEach(route=>{
      if(route.test()){
        route.activate(fresh)
      } else route.deactivate()
    })

    document.querySelectorAll('nonio-route[default]').forEach(defRoute => {
      let siblingsActivated = false
      let siblingRoutes = Array.from(defRoute.parentNode.children).filter(el => el!=defRoute && el.tagName == 'NONIO-ROUTE')
