...
**Tests:** seven new cases in `soci-frontend/test/browser.test.js`, fixturing the
video CDN with real h264 clips generated by ffmpeg (self-skipping when ffmpeg is
absent, as the suite already does for Chrome). Five of the seven fail against the
pre-fix component. Frontend `npm test` 35/35.
