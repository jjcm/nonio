...
## 2026-08-20 — Speed lab Qwen track: iteration 28 (KEEP — biggest win since iter14)
- Hypothesis (iter28 prompt): the :4200 handler and the backend Go httpd produce zero `Content-Encoding` (pre-verified) — every load ships ~90.6 KB of raw text (30.3 KB non-cacheable HTML doc + 49 KB `soci.css` + 11.3 KB `soci.js`). Gzip via built-in `zlib` should collapse it (measured: 21.0 KB on the wire, 3.7×/5.4×/3.1×) at zero client cost.
- Change (`soci-frontend/index.js`, +15/−5): `GZIP_EXT` (.js/.css/.html/.svg only) + `maybeGzip()` helper (`zlib.gzipSync`, gated on `Accept-Encoding`, sets `Content-Encoding: gzip` + `Vary: Accept-Encoding` before writeHead) applied at the three text send sites — static-file handler (iter14 `immutable` Cache-Control preserved), folder-handler `index.pug` doc, SPA-fallback `index.pug` doc. mp4 range path + :4201 API untouched. Smoke: 21 posts; gzip round-trip byte-identical for `/` and `/soci.css`; headers verified with and without `Accept-Encoding`.
- Measured via `run_baseline.py`: cold FCP 427.8 (−178.6) / cold LCP 802.8 (−158.6); warm FCP 177.0 ×3 (−40.0, zero spread — tightest FCP cluster in the lab) / warm LCP 430.1 (−40.0). TBT 0 on all loads; the 958/1056 cold and 450/470/530 warm bimodal bands that pinned every post-iter14 revert are gone (cold LCP spread 26 ms, warm 0.3 ms).
- **KEPT and committed** as `qwen-iter28` (superrepo gitlink bump + submodule `62753f3`). New Qwen-track record: **cold 427.8/802.8, warm 177.0/430.1** — this is now the keep bar for all future iterations. Lab record + raw metrics in `speed-lab/SPEED_LAB.md` / `speed-lab/metrics/baseline-iter28.json`.
- Key finding: the pinned-band diagnosis (iter25/27) held — timing levers over unchanged bytes were exhausted, so the bytes themselves were the last "small" lever. New floors visible: warm FCP 177.0 is the still-non-cacheable ~8.3 KB gzipped doc handoff (cache-on-doc = stale-feed product call, not a small lever); remaining byte levers (brotli, content-hashed names) are dependency/build-pipeline scale.

## 2026-08-20 — Speed lab Qwen track: iteration 29 (REVERT)
- Hypothesis (iter29 prompt, preferred branch): prefer brotli over gzip when `Accept-Encoding` includes `br` on the :4200 static handler — built-in `zlib.brotliCompressSync` (no dependency), existing `gzipSync` branch kept verbatim as fallback, iter14 immutable headers + iter28 gzip path untouched.
- Change (`soci-frontend/index.js`, +8/−3 in `maybeGzip()` only): `br` → `Content-Encoding: br`, else the unchanged gzip branch; `Vary: Accept-Encoding` on either path. Smoke: 21 fixture posts; br vs gzip fallback curl-verified on js/css/doc; br payload decodes; `immutable` rides the br response.
- Measured via `run_baseline.py` vs bar (cold 427.8/802.8, warm 177.0/430.1): cold FCP 595.9 (+168.1) / cold LCP 1061.9 (+259.1); warm FCP 177.0 ×3 (tie) / warm LCP 429.7 (−0.4).
- **Reverted** (`index.js` restored to submodule `62753f3`, served bytes confirmed brotli-free); post-revert confirmation sweep reproduces the bar (cold FCP 428.0 median, cold LCP 822.2 median; warm 177.0 ×3, warm LCP 430.1 median). Keep bar unchanged: cold 427.8/802.8, warm 177.0/430.1 (iter28).
- Key finding: brotli's ~15% byte saving (soci.js 3.64→3.09 KB, soci.css 9.04→7.71 KB) is real but `brotliCompressSync` at default quality 11 costs ~165 ms FCP / ~260 ms LCP of per-load encode CPU — pushing cold LCP back into the pre-iter28 1038–1071 pinned band. At ~30-49 KB payload scale the algorithm tax exceeds the saving; brotli would only re-enter as precomputed encoded artifacts (build-pipeline scale). Full log in `speed-lab/SPEED_LAB.md`; with-change raws at `/tmp/iter29-*.json`.

## 2026-08-20 — Speed lab Qwen track: iteration 30 (KEEP)
- Hypothesis (iter30 prompt, "else" branch): gzip `markdown.wasm` (56,655 B — the largest artifact on the cold wire, fetched per cold load on the LCP critical path since feed render waits on `markdown.ready`). The memoize/precompute-gzip alternative was rejected up front by bench: `gzipSync` at 49 KB costs ~0.64 ms — below the noise floor, so a path-cost lever cannot satisfy the all-lanes rule while a byte saving can (iter28 proof).
- Change (`soci-frontend/index.js`, +1/−1): `GZIP_EXT` gains `'.wasm': 1`; everything else (maybeGzip, send sites, iter14 immutable — `.wasm` already in `IMMUTABLE_EXT`, `application/wasm` Content-Type) untouched. Smoke: 21 posts; gz headers verified; decoded bytes identical to raw.
- Measured via `run_baseline.py` vs bar (cold 427.8/802.8, warm 177.0/430.1): cold FCP 427.9 (+0.1, exact tie) / cold LCP **783.0 (−19.8)** — new median below the entire pre-change distribution (old 797.9–824.2, new 762.0–798.9); warm FCP 177.0 ×3 (tie) / warm LCP 429.9 (−0.2, noise); TBT 0, score 1.00 across all seven loads.
- **KEPT and committed** as `qwen-iter30`. New Qwen-track record / keep bar: **cold 427.9/783.0, warm 177.0/429.9**. Full log in `speed-lab/SPEED_LAB.md`; raws in `speed-lab/metrics/baseline-iter30.json`.
- Key finding: the last un-gzipped text-ish artifact on the cold critical path is now compressed, so the remaining byte levers are either product calls (cache the non-cacheable doc) or already-closed (brotli tax, content-hash names = build-pipeline scale).

## 2026-08-20 — Speed lab Qwen track: iteration 31 (KEEP)
- Hypothesis (iter31 prompt): memoize per-file gzip in the :4200 static handler — every cold load re-fetches ~65 gzip-eligible artifacts (61 js + 1 css + 1 wasm + 2 svg per cold-1.json network pass) and each one paid `zlib.gzipSync` on the server per request. Iter30's per-artifact rejection (~0.64 ms each, below noise) was wrong in aggregate: 65 serial encodes per load is real response-latency tax on the assets the feed waits for.
- Change (`soci-frontend/index.js`, +34/−14): `GZIP_CACHE` keyed by path storing `{mtime, size, raw, gz: gzipSync(raw)}`; non-range file handler stats per request, serves cached Buffer on mtime+size match (no `readFile`, no `gzipSync`), reads+compresses once on miss/invalidation. `gzipSync` output verified deterministic in Node (zeroed MTIME) → wire bytes byte-identical. HTML/pug send sites keep per-request `maybeGzip`. Smoke: 21 posts; gz/raw/decoded sha256 all consistent; wasm gunzip `cmp` clean. Server restarted (PID 18549).
- Measured via `run_baseline.py` vs bar (cold 427.9/783.0, warm 177.0/429.9): cold FCP **423.1 (−4.8)** / cold LCP 784.9 (+1.9, sub-2 ms tie, straddles the iter30 distribution); warm FCP 177.0 ×3 (exact tie) / warm LCP 429.8 (−0.1). TBT 0, score 1.00 all seven loads.
- **KEPT and committed** as `qwen-iter31`. New Qwen-track record / keep bar: **cold 423.1/784.9, warm 177.0/429.8**. Full log in `speed-lab/SPEED_LAB.md`; raws in `speed-lab/metrics/baseline-iter31.json`.
- Key finding: the per-request encode tax only shows up on the cold lane (warms are `immutable`-cached), and only as an aggregate over the ~65-deep artifact waterfall — a sub-ms saving per artifact that no single-artifact lever could express. Combined with iter28/30: the remaining levers are now product calls (cache the non-cacheable doc) or dependency/build-pipeline scale.
