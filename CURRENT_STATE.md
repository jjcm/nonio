## 2026-08-25 — Video post detail: letterboxed and squashed picture (branch `cursor/fix-video-aspect-ratio-64bb`, stacked on `cursor/fix-media-cls-and-login-click-dd0b`)

Reported against https://non.io/avo-coffeeshop: a large black player, the video
letterboxed inside it, and the dog itself flattened horizontally. Three separate
causes, all of which had to go.

**1. The ratio box was inside the player instead of being the player.**
`soci-video` kept `:host { width: 100% }` and put the ratio-locked `#frame`
inside it, so a 9:16 video reserved a 450px box centred in an 800px black host —
and `controls`, absolutely positioned against the host, spanned the black rather
than the picture. The host is now the box (`:host([ratio])` carries
`MEDIA_BOX_CSS`); `#frame` just fills it. Fullscreen puts the box back inside the
host, since the screen has a ratio of its own.

**2. Portrait videos were being served a rendition they did not need.** The
resolution ladder is keyed on the encoder's own metric, `max(width, height)` of
the source, but `_adjustResForScreenSize` compared those rungs against
`this.offsetWidth`. Once the host became the box, a 450x800 portrait player
measured 450 and dropped to 480p on a 1080p desktop. It now compares against
`max(offsetWidth, offsetHeight)`, which leaves landscape behaviour identical.

**3. One rendition on the CDN is genuinely mis-encoded.**
`avo-coffeeshop-480p.mp4` is 1518x854 — landscape, holding the 720x1280 portrait
frames scaled non-uniformly, which is the flattened dog. Every other rendition
probed across the video posts on non.io is correct, so this is a legacy artifact
of `soci-video-cdn`'s `-s WxH` encode rather than a live encoder bug. Nothing in
the frontend can unsquash it, so `soci-video` now checks each rendition's frame
size against the ratio it locked; one that disagrees is taken off the ladder and
out of the resolution menu, and the source is played instead.

Ratio sources for video are now stored `width`/`height` to reserve the box, then
the file's own `videoWidth`/`videoHeight` at `loadedmetadata` to lock it. The
poster's ratio is no longer used at all: a video thumbnail is a crop of a frame
rather than a scaled copy, and avo-coffeeshop's is 615x545 for a 720x1280 video.
Image CLS locking and the feed tiles are untouched, both pinned by tests.

**4. An expanded feed tile could not be clicked on its own media.**
`soci-post-li`'s `#preview` is an overlay the size of the whole tile, holding the
full-resolution copy that is what you actually see once a tile is expanded. The
whole overlay carried `pointer-events: none`, so clicks fell through the copy to
the 96x72 thumbnail stacked underneath it; expanding and collapsing only worked
because both carry the same handler. `#preview img` now takes `pointer-events:
auto` while expanded, the same container-transparent / child-interactive pattern
`soci-post-list`, `soci-comment` and `soci-message-row` already use.

The wrapper itself deliberately stays `none`. Measured on the running feed, it is
1208x400 against a text column starting at x=1085, so making the wrapper a target
puts it over the title, tags and description — `elementFromPoint` at the title
returns `#preview`. Narrowing it instead does not work either: `width: auto`
takes the media from 496px to 237px, because `:host([expanded]) img`'s
`max-width: calc(100% - 280px)` resolves against the wrapper, so shrinking the
wrapper shrinks the copy off the thumbnail it is stacked on.

**Tests:** ten new cases in `soci-frontend/test/browser.test.js`. Seven cover the
video box, fixturing the video CDN with real h264 clips generated by ffmpeg
(self-skipping when ffmpeg is absent, as the suite already does for Chrome); five
of those fail against the pre-fix component. Three cover the expanded tile's hit
testing, one of which fails against the pre-fix component. Frontend `npm test`
38/38.
