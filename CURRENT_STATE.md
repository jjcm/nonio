...
- Shell-level API prefetch (`window.__preFetch` in index.pug, consumed by
  `nonio-post-list._loadPosts` + `nonio-component.getData`): home/tag/user feeds
...
- **Frontend perf:** back-navigation to the same feed reattaches the previous
  `nonio-post-list` (5-minute bound) instead of remounting — verified live:
  ...
  (`nonio-route.currentDom` cleared on fresh activation). `nonio-post-list`
...
- **Frontend (nonio-frontend)**
  - `index.js`: gzip for text/JSON/JS/CSS/SVG over 512 B, with output memoized
    ...
    without a restart.
  - `pages/user.pug`: dropped a bare `nonio-post-list` placeholder that fetched
    the entire unfiltered frontpage feed and was then discarded by
    `user.renderContent()`.
  - `nonio-post-list`: suppressed the duplicate feed request that
    `attributeChangedCallback('filter')` issued during `_initializeControls()`.
    API requests per navigation: user 6 → 3, tag 2 → 1.
  - `nonio-post`: reveal on the next frame instead of after a fixed
  ...
  - `nonio-comment-list`: build the comment tree before fetching
  ...
  - Entrance animations on the feed and the post page now ease opacity on
    `--nonio-ease-out` instead of the symmetric `--nonio-ease`. Durations and
...
- **Frontend (nonio-frontend)**
  ...
  - Registered `nonio-emoji` globally in `components/nonio-components.js`.
  - Migrated custom emoji usage to `nonio-emoji` in:
    - markdown rich-text decoration (`lib/nonio-rich-text.js`),
    - text-channel reaction chips and picker (`components/nonio-text-channel-view-threaded.js`),
    - inline token rendering + emoji suggestion rows in `nonio-input` (`components/nonio-input.js`),
    - personal/community emoji admin grids (`pages/admin/emojis.js`, `pages/community-emojis.js`).
  - Updated emoji CSS selectors to style `nonio-emoji` hosts in emoji grids.
  - Follow-up polish:
    - removed legacy `.emoji` class attachment from `nonio-emoji` usage paths,
    - sized markdown paragraph emojis to `21px` (`nonio-markdown-view p nonio-emoji`),
    - sized reaction-chip emojis to `14px` (`.reaction nonio-emoji` in text-channel view).
...
- **Frontend (nonio-frontend)**
  ...
  - Removed textarea-specific resize behavior in the text-channel composer (now no-op with `nonio-input`-managed editing).
  - Added public `insertText(...)` method to `components/nonio-input.js` for programmatic token insertion from external controls (emoji picker).
...
- **Frontend (nonio-frontend)**
  ...
  - Refactored `components/nonio-input.js` from `textarea` to a `contenteditable` editor that:
...
- **Frontend (nonio-frontend)**
  ...
  - New user panel includes:
    - user switcher header with selected `nonio-user`
  ...
  - Added sidebar panel styling for the new user panel and simplified old `nonio-route#user` layout rules in `nonio.css`.
  - Added `nonio-sidebar-user-panel` to sidebar markup (`sidebar.pug` and `index.html`).
  - Follow-up: converted user-panel actions (`edit profile`, `view financials`, `posts`, `comments`) to `nonio-tag-li`.
...
- **Frontend (nonio-frontend)**
  ...
  - DRY-ed CSS: consolidated duplicate `h2`/`nonio-tag-li` rules for `.admin-links` and `#user-content-nav` into a shared rule.
  - Removed unused `config` import from `nonio-sidebar-panel.js`.
...
- **Frontend (nonio-frontend)**
  ...
  - `nonio-post-card` now uses the post list payload for descriptions (no per-card network request); masonry relayout is triggered on image/markdown load via `card-loaded`
  - `nonio-post-card`: clicking media now navigates to the post
  - Extracted the post timestamp clock SVG into `nonio-icon` as `time` glyph; `nonio-post-li` now uses the shared icon
...
- **Frontend (nonio-frontend)**
  ...
  - Added `fetchAndMerge()` method to `nonio-post-list.js`:
...
- **Frontend (nonio-frontend)**
  ...
  - Fixed `nonio-post-list` first child margin: removed margin-top from first `nonio-post-li`, increased top padding to 12px
...
- **Frontend (nonio-frontend)**
  ...
  - **Added `nonio-post-card.js`**: New card component for masonry view (extends `nonio-post-li`)
    - Different element order: Title → Media → Description (markdown) → Details bar → Tags
    - Fetches and renders post content as `nonio-markdown-view`
  ...
  - Updated `nonio-post-list.js`:
    - Added `view` attribute (`list` | `lanes`) to toggle between `nonio-post-li` and `nonio-post-card`
  ...
  - Updated `nonio-column.js`:
    ...
    - Toggle persists view preference to child `nonio-post-list`
    - Hidden on mobile (< 768px) to save header space
  - Added card markdown styles to `nonio.css` (h1-h3 reduced to 14px, compact paragraphs)
...
- **Frontend (nonio-frontend)**
  ...
  - Hid delete link on `nonio-post-li` elements in mobile view to prevent accidental deletions and save space
  - Stacked `.sidebar` and `.content` vertically on the user page (`nonio-route#user`) for mobile view, allowing natural scrolling of the profile followed by their posts/comments
...
- **Frontend (nonio-frontend)**
  ...
  - Updated modal panel logic to resolve sidebar context from either local ancestry or global `nonio-sidebar`, so existing login/register/create-community submit behavior still calls shared sidebar refresh/auth flows.
  - Added explicit modal-scoped CSS for the moved flows in `nonio.css`, including login footer link styling and create-community form/error styles.
  - Fixed `nonio.showRegister()` to call the real sidebar API (`showCreateAccount`) instead of a non-existent `_createAccount()` method.
...
- **Frontend (nonio-frontend)**
  ...
  - Updated `nonio-modal` to dispatch lifecycle events (`modalactivate`, `modaldeactivate`) so modal cleanup can be event-driven without mutation observers.
  - Simplified `nonio-sidebar` modal integration: removed observer-based syncing and legacy panel activation toggling; sidebar now delegates to modal manager APIs directly.
  - Removed legacy auth/create panel classes from `components/nonio-sidebar-panel.js`; file now contains sidebar panel components only.
...
- **Frontend (nonio-frontend)**
  ...
  - Centralized component registration: modals are now imported and defined in `nonio-components.js` (eagerly loaded) rather than self-defining or lazy-loading, matching the project pattern.
  - Updated `nonio-modal-manager.js` to support a `register()` API (extensibility) and updated its default registry to use the eagerly-loaded components.
  - **UX Improvements**:
    - `nonio-create-account-modal`: Added Enter key submission support and auto-close (500ms delay) on success, matching login behavior.
    - `nonio-create-community-modal`: Added Enter key submission support. 
